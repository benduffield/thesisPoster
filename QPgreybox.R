...
CVS <- function(time, u, params) {
  ...
  with(as.list(params), {
    ...
    VsptQP = pred_mean_QP(VrvQP,Vrv_obs,VlvQP,Vlv_obs,time,tsteps_obs,Vspt_obs)
...
original_data_QP <- as.data.frame(sol)

# Adding noise to data
noise_magnitude <- 0.0
sd_vals <- apply(original_data_QP, 2, sd)
noisy_data <- as.data.frame(sweep(original_data_QP, 2, noise_magnitude * sd_vals, "+") + 
                              noise_magnitude * matrix(rnorm(nrow(original_data_QP) * ncol(original_data_QP)), 
                                                       nrow = nrow(original_data_QP), ncol = ncol(original_data_QP)))


noisy_data_tb_QP <- as_tibble(noisy_data) %>%
  pivot_longer(cols = QmtQP:VpuQP, 
               names_to = "variable", 
               values_to = "vals")

p_noisy_data_tb_QP <- ggplot(noisy_data_tb_QP) +
  geom_line(aes(x = time, y = vals), color = "black", lwd = 1) +
  facet_grid(vars(variable), scales = "free")


p_noisy_data_tb_QP + p_noisy_data_tb

ggplot() + 
  geom_line(aes(x = tsteps, y = Qav)) + 
  geom_line(aes(x = tsteps, y = QavQP), col = "blue")

# Vspt and calculation
QmtQP <- noisy_data$QmtQP
QavQP <- noisy_data$QavQP
QtcQP <- noisy_data$QtcQP
QpvQP <- noisy_data$QpvQP
VlvQP <- noisy_data$VlvQP
VaoQP <- noisy_data$VaoQP
VvcQP <- noisy_data$VvcQP
VrvQP <- noisy_data$VrvQP
VpaQP <- noisy_data$VpaQP
VpuQP <- noisy_data$VpuQP
...
VsptQP <- numeric(length(tsteps))

for (i in seq_along(tsteps)) {
  mean_conditional = pred_mean_QP(VrvQP[i],Vrv_obs,VlvQP[i],Vlv_obs,tsteps[i],tsteps_obs,Vspt_obs)
  cov_conditional = pred_cov_QP(VrvQP[i],Vrv_obs,VlvQP[i],Vlv_obs,tsteps[i],tsteps_obs)
  
  VsptQP[i] = mean_conditional
}
