...
library(reticulate)
#-----------------------------------------
#Creating model - run NR for 0.75s using 30 time steps
#-----------------------------------------

playground_model = cmdstan_model(stan_file = "EstimatedPeriodQPKernel.stan")
...
period_MAP = as.double(MAP_params$summary("p")[2])
...
mcmc_trace(param_samples$draws(),pars = c("sigma", "length_scale1", "length_scale2","length_scale3","p"))
param_samples
param_samples$draws()
...
ggplot() + 
  ...
  labs(title = "GP predictions with period p = 0.0681",
       x = "Time (s)",
       y = "Vspt (ml)") +
