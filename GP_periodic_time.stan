...
model {
  ...
  // Compute the covariance matrix using a QP kernel
  for (i in 1:N) {
    for (j in 1:N) {
      K[i, j] = ((exp(-square(x1[i] - x1[j]) / (2 * square(length_scale1)))) * (exp(-square(x2[i] - x2[j]) / (2 * square(length_scale2)))) * (exp((-2/square(length_scale3)) * square(sin(pi() * abs(x3[i]-x3[j])/0.75)))));
      if (i == j){
      K[i,j] = K[i,j] + 1e-7;
      }
    }
