...
original_data_QPm3 <- as.data.frame(sol)

# Adding noise to data
noise_magnitude <- 0.0
sd_vals <- apply(original_data_QPm3, 2, sd)
noisy_data <- as.data.frame(sweep(original_data_QPm3, 2, noise_magnitude * sd_vals, "+") + 
                              noise_magnitude * matrix(rnorm(nrow(original_data_QPm3) * ncol(original_data_QPm3)), 
                                                       nrow = nrow(original_data_QPm3), ncol = ncol(original_data_QPm3)))


noisy_data_tb_QPm3 <- as_tibble(noisy_data) %>%
  pivot_longer(cols = QmtQP:VpuQP, 
               names_to = "variable", 
               values_to = "vals")

p_noisy_data_tb_QPm3 <- ggplot(noisy_data_tb_QPm3) +
  geom_line(aes(x = time, y = vals), color = "black", lwd = 1) +
  facet_grid(vars(variable), scales = "free")


p_noisy_data_tb_QPm3
...
QmtQPm3 <- noisy_data$QmtQP
QavQPm3 <- noisy_data$QavQP
QtcQPm3 <- noisy_data$QtcQP
QpvQPm3 <- noisy_data$QpvQP
VlvQPm3 <- noisy_data$VlvQP
VaoQPm3 <- noisy_data$VaoQP
VvcQPm3 <- noisy_data$VvcQP
VrvQPm3 <- noisy_data$VrvQP
VpaQPm3 <- noisy_data$VpaQP
VpuQPm3 <- noisy_data$VpuQP
...
VsptQPm3 <- numeric(length(tsteps))

for (i in seq_along(tsteps)) {
  mean_conditional = pred_mean_QP(VrvQPm3[i],Vrv_obs,VlvQPm3[i],Vlv_obs,tsteps[i],tsteps_obs,Vspt_obs)
  cov_conditional = pred_cov_QP(VrvQPm3[i],Vrv_obs,VlvQPm3[i],Vlv_obs,tsteps[i],tsteps_obs)
  
  VsptQPm3[i] = mean_conditional - (3 * sqrt(cov_conditional))
}
